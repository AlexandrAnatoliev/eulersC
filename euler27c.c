...
void *my_realloc(char *arr, int *len_arr, size_t len_add);
int note_comp_arr(char *comp_arr, int start, int finish);
bool is_composite(char comp_arr[], int num);
int note_composite(char comp_arr[], int prime, int start, int finish);
...
int main(void)
{
    ...
    int len_arr = 1000; // длина массива
    // size_t это typedef unsigned long long size_t - тип данных для sizeof
    int n, answ_a, answ_b, a, b, cnt, answ = 1, cnt_max = 0;

    char *comp_arr = calloc(len_arr, sizeof(char)); // массив для хранения составных чисел

    note_comp_arr(comp_arr, 2, len_arr); // заполняем массив составными числами
    // for (int i = 9000; i < len_arr; i++)
    //     if (!comp_arr[i])
    //         printf("%d ", i);
    ...
    //             if (answ < 0 || comp_arr[answ] == 1) // простое число не может быть отрицательным
    ...
    for (int a = -1000; a < 1000; a++)
    {
        ...
        {
            int cnt = 0;
            for (int n = 0; n < 100; n++) // предположил, что простых чисел будет не более ста
            {
                int answ = (n * n) + (a * n) + b; // проверяем формулу

                if (answ >= len_arr)
                {
                    int past_len = len_arr;                                           // сохраняем изначальную длину массива
                    comp_arr = my_realloc(comp_arr, &len_arr, answ - len_arr + 1000); // т.к. len_arr  увеличивается в функции
                    note_comp_arr(comp_arr, past_len, len_arr);                       // отмечаем в добавленной части массива составные числа
                    printf("%d\n", len_arr);
                }
...
// return:      указатель на увеличенный массив
{
    ...
    for (int i = 0; i < *len_arr; i++)
        new_arr[i] = arr[i]; // копируем данные вновый массив из старого
...
{
    // отмечаем составные числа на основе уже найденных простых
    for (int num = 2; num < start; num++) // если число не отмечено составным (простое)
        if (!comp_arr[num])
            note_composite(comp_arr, num, start, finish); // отмечаем составные числа, кратные простому
    ...
    for (int num = start; num < finish; num++) // определяем список простых чисел (!составных)
    {
        ...
        if (!comp_arr[num] && !is_composite(comp_arr, num))
            note_composite(comp_arr, num, start, finish); // отмечаем составные числа, кратные простому
    }
    return 0;
}
