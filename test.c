...
typedef enum {
    ST_HIGHEST, // Highest value card.
    ST_PAIR, // Two cards of the same value.
    ST_TWO_PAIRS, // Two different pairs.
    ST_THREE, // Three cards of the same value.
    ST_STRAIGHT, // All cards are consecutive values.
    ST_FLUSH, // All cards of the same suit.
    ST_FULL_HOUSE, // Three of a kind and a pair.
    ST_FOUR, // Four cards of the same value.
    ST_STRAIGHT_FLUSH, // All cards are consecutive values of same suit.
    ST_ROYAL_FLUSH // Ten, Jack, Queen, King, Ace, in same suit.
} StateEnum;
...
typedef struct {
    ...
    int state_value;
    char sorted_vals[5];
    char value_counts[13];
    char last_suit;
    char suit_cnt;
} Hand;

char get_val(char val)
{
    switch (val) {
        case '2': return 0;
        case '3': return 1;
...
void adjust_state_straight(Hand* hand)
{
    char *sv = hand->sorted_vals;
    if (
        sv[0] - sv[1] == 1 && sv[1] - sv[2] == 1
        && sv[2] - sv[3] == 1 && sv[3] - sv[4] == 1
    ) {
        hand->state = ST_STRAIGHT;
    }
}

void adjust_state_suit(Hand *hand)
{
    if (hand->state == ST_STRAIGHT) {
        if (hand->value_counts[12]) {
            hand->state = ST_ROYAL_FLUSH;    
        } else {
            hand->state = ST_STRAIGHT_FLUSH;
        }
    } else {
        hand->state = ST_FLUSH;
    }
...
void adjust_state(Hand* hand, char val, int card_idx)
{
    switch (hand->value_counts[val]) {
        case 2:
            if (hand->state == ST_PAIR) {
                hand->state = ST_TWO_PAIRS;
                if (hand->state_value < val) {
                    hand->state_value = val;
                }
            } else if (hand->state == ST_THREE) {
                hand->state = ST_FULL_HOUSE;
            } else {
                hand->state = ST_PAIR;
                hand->state_value = val;
            }
        ...
        case 4:
            ...
            break;
        default:
            if (hand->state == ST_HIGHEST) {
                if (hand->state_value < val) {
                    hand->state_value = val;
                }
                if (card_idx == 4) {
                    adjust_state_straight(hand);
                }
                if (hand->suit_cnt == 5) {
                    adjust_state_suit(hand);
                }
            }
    }
}

void hand_add_val(Hand* hand, char val, int i)
{
    while (i > 0) {
        if (hand->sorted_vals[i - 1] < val) {
            hand->sorted_vals[i] = hand->sorted_vals[i - 1];
            i--;
        } else {
            break;
        }
    }
    hand->sorted_vals[i] = val;
    hand->value_counts[val]++;
}

void parse_hand(Hand* hand, char* input)
{
    memset(hand, 0, sizeof(Hand));
    hand->state = ST_HIGHEST;
    hand->last_suit = input[1];
    for (int i = 0; i < 5; i++) {
        char val = get_val(*input);
        if (hand->last_suit == input[1]) {
            hand->suit_cnt++;
        }
        input += 3;
        hand_add_val(hand, val, i);
        adjust_state(hand, val, i);
    }
}

int compare_states(Hand* hand1, Hand* hand2)
{
    if (hand1->state_value > hand2->state_value) {
        return 1;
    } else if (hand1->state_value < hand2->state_value) {
        return 0;
    }
    ...
    int i2 = 0;
    while (i1 < 5 && i2 < 5) {
        if (hand1->sorted_vals[i1] == hand1->state_value) {
            i1++;
        }
        if (hand2->sorted_vals[i2] == hand2->state_value) {
            i2++;
        }
        if (hand1->sorted_vals[i1] > hand2->sorted_vals[i2]) {
            return 1;
        }
        if (hand1->sorted_vals[i1] < hand2->sorted_vals[i2]) {
            return 0;
...
void main()
{
    ...
    while (ptr < 6) {
        parse_hand(&hand1, c);
        parse_hand(&hand2, c + 15);
        c += 30;
        // ptr+=30;
        if (hand1.state > hand2.state) {
            cnt++;
            // printf("%d\n", cnt);
        } else if (hand1.state == hand2.state) {
            cnt += compare_states(&hand1, &hand2);
            // printf("%d\n", cnt);
        }
        ptr++;
    }
...
static char* hands = "8C TS KC 9H 4S 7D 2S 5D 3S AC \
